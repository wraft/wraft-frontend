...
export const mealOrderSelection = (
  _startDate: any,
  endDate: any,
  menu: any,
) => {
  const activeWeek =
    menu && menu.filter((item: any) => item.current_week == true);
  ...
  let month_days: any = [];
  month_weekdays &&
    month_weekdays.forEach((data: any) => {
...
export const dateGroupArry = (startDate: any, endDate: any) => {
  ...
  const remap =
    resultto &&
    resultto.map((item) => ({
      date: format(item, 'dd LLL yyyy'),
      day: format(item, 'iiii').toLowerCase(),
    }));
