...
import { BLOCKR_FILE_TYPES } from './blockr-constants';

/**
 * Whether the file is an image file
 */
export const isImageFileType = (file: File) => {
  return BLOCKR_FILE_TYPES.has(file.type);
};
...
export const getAttrs = (extraAttrs: Attrs) => (domNode: HTMLElement) => {
  ...
  let align = domNode.getAttribute('data-align') ? domNode.getAttribute('data-align') : domNode.getAttribute('align');
  if (align) {
  ...
  return {
    ...extraAttrs,
    align,
    alt: domNode.getAttribute('alt') ? domNode.getAttribute('alt'): null,
    crop,
    height: parseInt(height || '0', 10) || null,
    rotate,
    src: domNode.getAttribute('src') ? domNode.getAttribute('alt') : null,
    title: domNode.getAttribute('title') ? domNode.getAttribute('alt') : null,
