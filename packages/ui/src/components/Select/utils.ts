...
export const itemToString = (item: Option): string =>
  item ? item.label : EMPTY_STRING;

export const getSpacer = (options: Options): string => {
  return options.reduce(
    (prev: string, current: OptionItem) =>
      current.label && prev.length > current.label.length
        ? prev
        : current.label,
    EMPTY_STRING,
  );
...
export const isValueSelected = (
  value: OptionValue,
  options: Option[],
): boolean => !!options.find((item) => item.value === value);
...
export const getOption = (
  ...
  return (
    (option as Option) || ({ value: kebabCase(value), label: value } as Option)
  );
};
...
export const getNewOptions = (
  ...
  return selected.filter(
    (item) => !options.find((option: Option) => option.value === item.value),
  );
};
